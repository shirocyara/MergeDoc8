...
public class FastStringUtils {
	...
	/** ロガー */
	private static final Logger logger = LogManager.getLogger(FastStringUtils.class);
	 ...
	 * 
	 * <p>
	 * これは以下のような JDK の String#replace(CharSequence,CharSequence)
	 * メソッドと処理内容は同等ですが、内部的に使用する Pattern オブジェクトを キャッシュするため、同じ target の場合、2
	 * 回目以降は高速です。
	 * 
	 * <pre>
	 ...
	 * 
	 * 注意：<br>
	 * target が毎回異なるようなケースでは、無駄な Pattern オブジェクトが キャッシュされるため、使用しないようにしてください。
	 * 
	 * @param input
	 *            マッチされる文字列
	 * @param target
	 ...
	 * 
	 * <p>
	 * これは以下のような JDK の String#replaceFirst(String,String)
	 * メソッドと処理内容は同等ですが、内部的に使用する Pattern オブジェクトを キャッシュするため、同じ regex の場合、2
	 * 回目以降は高速です。
	 * 
	 * <pre>
	 ...
	 * 
	 * 注意：<br>
	 * regex が毎回異なるようなケースでは、無駄な Pattern オブジェクトが キャッシュされるため、使用しないようにしてください。
	 * 
	 * @param input
	 *            マッチされる文字列
	 * @param regex
	 ...
	 * 
	 * <p>
	 * これは以下のような JDK の String#replaceAll(String,String) メソッドと処理内容は同等ですが、内部的に使用する
	 * Pattern オブジェクトを キャッシュするため、同じ regex の場合、2 回目以降は高速です。
	 * 
	 * <pre>
	 ...
	 * 
	 * <p>
	 * これは以下のような JDK の String#matches(String) メソッドと処理内容は同等ですが、内部的に使用する Pattern
	 * オブジェクトを キャッシュするため、同じ regex の場合、2 回目以降は高速です。
	 * 
	 * <pre>
	 * input.matches(regex);
	 * </pre>
	 ...
	 * 
	 * <p>
	 * これは以下のような JDK の String#split(String,int) メソッドと処理内容は同等ですが、内部的に使用する Pattern
	 * オブジェクトを キャッシュするため、同じ regex の場合、2 回目以降は高速です。
	 * 
	 * <pre>
	...
	/**
	 * 正規表現のメタ文字を \ でエスケープします。 対象となるメタ文字はエスケープ文字 \ および前方参照の部分参照を示す $ です。
	 * 
	 * <p>
	 * これは JDK の Matcher.quoteReplacement(String) メソッドと処理内容は 同等ですが高速に動作します。
	 * 
	 * @param str
	 *            対象文字列
	 * @return エスケープした文字列
	...
	/**
	 * 文字列に含まれるタブをスペースに置換します。 今のところ、JDK のソースに合わせスペース 8 文字として処理します。
	 * 
	 * @param str
	 *            文字列
	...
	/**
	 * 文字列の高さ（行数）を求めます。 この値は文字列中に含まれる改行（LF）の数と等価です。
	 * 
	 * @param str
	 *            文字列
	 * @return 高さ（行数）
