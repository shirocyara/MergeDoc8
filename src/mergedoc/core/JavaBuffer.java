...
public class JavaBuffer {
	 ...
	 * ブロックコメント単位で走査し、そのコメントがどのクラス（インナークラス） に属するかを classBlock クラスのスタックで制御します。
	 * <p>
	 * しかし、インナークラス宣言に Javadoc コメントが無い場合があるので ここでダミー Javadoc
	 * コメントを挿入します（後で削除）。なお、Javadoc コメントが無い場合は Javadoc API ドキュメントも存在しません。
	 * 
	 * <pre>
	 ...
	 * で始まるクラスコメントがありますが、Javadoc コメントでないのに Javadoc API
	 * ドキュメントが存在します。今のところ、そのようなコメントは 日本語化の対象とはなりません。
	 * 
	 * @param src
	 *            Java ソース文字列
	...
	private String setupDummyComment(String src) {
		...
		for (int i = declareMaxLength; i <= last; i++) {
			...
			if (c[i] == ' ' || c[i] == '\n' || c[i] == '<') {
				...
				} else if (c[i - 9] == 'i' && c[i - 8] == 'n' && c[i - 7] == 't' && c[i - 6] == 'e' && c[i - 5] == 'r'
						&& c[i - 4] == 'f' && c[i - 3] == 'a' && c[i - 2] == 'c' && c[i - 1] == 'e') {
					// interface 宣言の場合
					if (c[i - 10] == ' ' || c[i - 10] == '\n') {
						declaPos = i - 11;
					} else if (c[i - 10] == '@') {
	...
	/**
	 * 次のブロックコメントがあるか判定します。 ある場合は現在位置をブロックコメントに移動します。
