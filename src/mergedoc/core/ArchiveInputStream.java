 ...
 * アーカイブ入力ストリームです。
 * <p>
 * 異なる形式のアーカイブファイル .zip や .tar.gz を同一視するためのクラスです。 扱いやすくするためにインターフェースは
 * ZipInputStream に合わせています。
 * <p>
...
abstract public class ArchiveInputStream {
	 ...
	 * インスタンスを返します。今のところ、形式の判定は簡易的にファイル拡張子で 行っており、データ内容での判定は一切行われません。
	 * 
	 * @param アーカイブファイル
	 * @return アーカイブ入力ストリーム
	 * @throws IOException
	...
	/**
	 * 入力ストリームからバイト配列に最大 byte.length バイトの データを読み込みます。このメソッドは入力データが読み込み可能になる
	 * までブロックします。
	 * <p>
	 * このメソッドは単純に read(b, 0, b.length) の呼び出しを 実行し、その結果を返します。代わりに in.read(b)
	 * が実行されな いようにしてください。FilterInputStream の特定のサブクラスは、 実際に使用されている実装方法に依存します。
	 * 
	 * @param b
	 *            データの読み込み先のバッファ
