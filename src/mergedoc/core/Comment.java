...
public class Comment {
  ...
  /**
   * 元の Java ソースコメントをセットします。 これには飾り付け部分が含まれます。
   * <p>
   * 
  ...
  /**
   * 元の Java ソースコメントから指定した Javadoc タグを探し、その値の リストを作成します。タグの値に改行が含まれる可能性があるものが対象です。
   * <p>
   * 
   * @param tagName タグ名
   * @return タグの値リスト
  ...
  /**
   * 元の Java ソースコメントから指定した Javadoc タグを探し、その値の リストを作成します。タグの値に改行がないものが対象です。
   * <p>
   * 
   * @param tagName タグ名
   * @return タグの値リスト
  ...
  /**
   * Java ソース上ではスーパークラスやインタフェースに Javadoc コメントがあれば省略することが可能ですが、API ドキュメントには すべて出力されています。よって、元々 Java
   * ソースに無いタグを API ドキュメントから取り込んでしまわないように API ドキュメントにタグがあっても Java ソースもタグが存在しなければ null を返します。
   * <p>
   * タグが複数存在する可能性がある場合は {@link #omitTags(List, String)} を使用してください。
   * <p>
   * 
  ...
  /**
   * Java ソース上ではスーパークラスやインタフェースに Javadoc コメントがあれば省略することが可能ですが、API ドキュメントには すべて出力されています。よって、元々 Java
   * ソースに無いタグを API ドキュメントから取り込んでしまわないように API ドキュメントのタグ数と Java ソースのタグ数を比較し、 API ドキュメントのほうが少なければ、Java
   * ソースのタグ数に合わせます。
   * <p>
   * 
   * @param docTagList API ドキュメントのタグリスト
  ...
  /**
   * JDK1.5 日本語 API ドキュメントの不具合？で pre HTML タグ内の値は 改行情報が欠落している場合があるため、Java ソースコメントのものを使用します。 ただし、pre
   * タグの個数が一致しない場合は、そのまま API ドキュメントの 値を使用します。
   */
  ...
  /**
   * タグコメントのリストを文字列バッファに追加します。 複数行になる場合はインデントします。
   * <p>
   * 
   * @param tag タグ文字列
  ...
  private String adjustWidth(String value, int width) {
    ...
    for (String lineValue : lineValues) {
      ...
      String multiLineValue =
          FastStringUtils.replaceAll(lineValue, "\\s?(\\p{Graph}{" + longWordWidth + ",})", "\n$1");
  ...
  /**
   * 単一行の文字列を指定した折り返し幅になるように改行を挿入します。 入力行の値により最大折り返し幅を超える場合があります。
   * <p>
   * 
   * @param lineValue 入力行
