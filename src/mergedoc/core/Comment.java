...
public class Comment {
	...
	/**
	 * Java ソース上ではスーパークラスやインタフェースに Javadoc コメントがあれば省略することが可能ですが、API ドキュメントには
	 * すべて出力されています。よって、元々 Java ソースに無いタグを API ドキュメントから取り込んでしまわないように API
	 * ドキュメントにタグがあっても Java ソースもタグが存在しなければ null を返します。
	 * <p>
	 ...
	 * Java ソース上ではスーパークラスやインタフェースに Javadoc コメントがあれば省略することが可能ですが、API ドキュメントには
	 * すべて出力されています。よって、元々 Java ソースに無いタグを API ドキュメントから取り込んでしまわないように API
	 * ドキュメントのタグ数と Java ソースのタグ数を比較し、 API ドキュメントのほうが少なければ、Java ソースのタグ数に合わせます。
	 * <p>
	 * 
	 * @param docTagList
	 *            API ドキュメントのタグリスト
	...
	/**
	 * JDK1.5 日本語 API ドキュメントの不具合？で pre HTML タグ内の値は 改行情報が欠落している場合があるため、Java
	 * ソースコメントのものを使用します。 ただし、pre タグの個数が一致しない場合は、そのまま API ドキュメントの 値を使用します。
	 */
	...
	private String resizeComment(OutputComment o, String decoComment) {
		...
		if (o.resultHeight() > o.originHeight) {
			...
			if (o.resultHeight() > o.originHeight) {
				...
				logger.warn(sig + " 行数調整不可のためマージ出来ませんでした。\n" + "-------------------------------------------------\n"
						+ "英語コメント:\n" + srcBody + "\n日本語コメント:\n" + o.toString()
						+ "-------------------------------------------------\n");
				return srcBody;
			}
