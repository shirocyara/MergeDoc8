...
public class ComponentFactory {
	...
	/**
	 * このアプリケーションがサポートするコンポーネント最大サイズを作成します。 この Dimension は通常の Dimension
	 * と異なりサイズを変更するメソッドを 呼び出してもサイズが変わらない不変クラスです。
	 * 
	...
	/**
	 * 指定したコンポーネント配列に設定されているフォントメトリクスから 最も長いテキストの幅に各コンポーネントの幅を合わせます。
	 * 
	 * @param compos
	 *            コンポーネント配列
	 */
	public static void ensureMaxFontWidth(JComponent[] compos) {
		...
		for (JComponent compo : compos) {
			...
			if (compo instanceof JLabel) {
				...
				ComboBoxModel<Object> model = ((JComboBox<Object>) compo).getModel();
