 ...
 * アーカイブ入力ストリームです。
 * <p>
 * 異なる形式のアーカイブファイル .zip や .tar.gz を同一視するためのクラスです。 扱いやすくするためにインターフェースは ZipInputStream に合わせています。
 * <p>
 * 汎用的にするために将来 java.io.InputStream インタフェースを実装する可能性が あります。
 * 
...
abstract public class ArchiveInputStream {
   ...
   * アーカイブ入力ストリームを作成します。
   * <p>
   * 指定されたファイルの形式を判定し、適切な ArchiveInputStream 実装クラスの インスタンスを返します。今のところ、形式の判定は簡易的にファイル拡張子で
  ...
  /**
   * 次のアーカイブファイルエントリを読み込み、エントリデータの最初にストリームを 配置します。
   * 
  ...
  /**
   * 入力ストリームからバイト配列に最大 byte.length バイトの データを読み込みます。このメソッドは入力データが読み込み可能になる までブロックします。
   * <p>
   * このメソッドは単純に read(b, 0, b.length) の呼び出しを 実行し、その結果を返します。代わりに in.read(b) が実行されな
   * いようにしてください。FilterInputStream の特定のサブクラスは、 実際に使用されている実装方法に依存します。
   * 
   * @param b データの読み込み先のバッファ
   * @return バッファに読み込まれたバイトの合計数。ストリームの終わりに 達してデータがない場合は -1
   * @throws IOException 入出力エラーが発生した場合
