...
import mergejdoc.xml.ConfigManager;
import mergejdoc.xml.ReplaceEntry;
import mergejdoc.xml.ReplaceHandler;
...
public class MergeManager {
  ...
  public void validate()
      throws MergeDocException, IOException, InterruptedException, ExecutionException {
  ...
  public void execute() throws MergeDocException, SAXException, IOException {
    ...
    try {
      ...
      out = new ZipOutputStream(
          new BufferedOutputStream(new FileOutputStream(pref.getOutputArchive())));
  ...
  /**
   * アーカイブ入力ストリームから順次エントリを読み込み、Java ソースの場合は API ドキュメントとマージし、それ以外のファイルはそのまま ZIP 出力ストリームに書き込みます。
   * 
   ...
   * @throws IOException 入出力例外が発生した場合
   */
  private String chkFilter(String entryName, String source)
      throws MergeDocException, SAXException, IOException {
    // クラス別置換定義の処理
