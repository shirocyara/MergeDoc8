...
public class JavaBuffer {
  ...
  /** ロガー */
  private static final Logger logger = LogManager.getLogger(JavaBuffer.class);
   ...
   * <p>
   * Javadoc コメントの取込みは Java ソースを nextComment メソッドで ブロックコメント単位で走査し、そのコメントがどのクラス（インナークラス） に属するかを
   * classBlock クラスのスタックで制御します。
   * <p>
   * しかし、インナークラス宣言に Javadoc コメントが無い場合があるので ここでダミー Javadoc コメントを挿入します（後で削除）。なお、Javadoc コメントが無い場合は
   * Javadoc API ドキュメントも存在しません。
   * 
   * <pre>
   ...
   * で始まるクラスコメントがありますが、Javadoc コメントでないのに Javadoc API ドキュメントが存在します。今のところ、そのようなコメントは 日本語化の対象とはなりません。
   * 
  ...
  private String setupDummyComment(String src) {
    ...
    for (int i = declareMaxLength; i <= last; i++) {
      ...
      if (c[i] == ' ' || c[i] == '\n' || c[i] == '<') {
        ...
        if (c[i - 5] == 'c' && c[i - 4] == 'l' && c[i - 3] == 'a' && c[i - 2] == 's'
            && c[i - 1] == 's') {
          // class 宣言の場合
          if (c[i - 6] == ' ' || c[i - 6] == '\n') {
            declaPos = i - 7;
  ...
  /**
   * 次のブロックコメントがあるか判定します。 ある場合は現在位置をブロックコメントに移動します。 ブロックコメントが連続している場合は最後のブロックコメントが有効になります。
   * 
  ...
  public Signature getSignature() {
    ...
    commentEndToEOF =
        FastStringUtils.replaceFirst(commentEndToEOF, "(?s)^(\\s*@[\\w]+\\s*\\(.*?\\))*\\s*", "");
    ...
    logger.warn("Javadoc コメントの後のシグネチャを取得できませんでした。\n" + commentEndToEOF);
    return null;
  }
  ...
  /**
   * 現在位置にローカライズされたブロックコメントをセットします。 指定されたブロックコメントが null の場合は無視します。
   * 
   * @param sig シグネチャ
