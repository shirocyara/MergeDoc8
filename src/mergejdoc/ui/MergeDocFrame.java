...
import mergejdoc.core.MergeManager;
import mergejdoc.core.Preference;
import mergejdoc.core.WorkingState;
...
public class MergeDocFrame extends JFrame {
  ...
  /** ロガー */
  private static final Logger logger = LogManager.getLogger(MergeDocFrame.class);
  ...
  /**
   * アプリケーションを終了します。 ウィンドウ状態や設定内容は設定ファイルに保存されます。
   */
  private void end() {
    persistent();
    try {
  ...
  /**
   * ウィンドウ状態を Persister にセットします。 ウィンドウリサイズ時やアプリケーション終了時に呼び出されます。
   */
  ...
  /**
   * このフレームのタイトルを、指定された文字列に設定します。 このメソッドはスレッドに対して安全です。
   * 
