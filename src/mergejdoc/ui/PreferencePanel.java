...
import mergejdoc.core.FastStringUtils;
import mergejdoc.core.Preference;
...
public class PreferencePanel extends JPanel {
  ...
  private JComponent createUpperPanel() throws MergeDocException {
    ...
    // コンボ設定
    JComboBox<Object> docCombo = docField.getComboBox();
    JComboBox<Object> srcCombo = srcField.getComboBox();
    ...
    if (targetStr != null) {
      ...
      if (!targetDir.exists() || targetDir.isFile()) {
        throw new MergeDocException(
            "オプション " + OPTION_KEY + " に指定された値 " + targetStr + " は\n存在しないかディレクトリではありません。");
      }
  ...
  /**
   * 指定したベースディレクトリを遡り、予想される入出力ソースアーカイブファイルを 設定します。入出力ソースアーカイブファイルのどちらか 1 つでも値が設定 されている場合は何も行いません。
   * 
