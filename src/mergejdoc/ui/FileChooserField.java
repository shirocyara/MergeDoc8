...
public class FileChooserField extends JPanel {
  ...
  /** コンボボックス部 */
  private final JComboBox<Object> combo = new JComboBox<Object>();
  ...
  /**
   * 文字セット名を昇順で保持するコンボボックスモデルです。
   * <p>
   * 
   * プラットフォームでサポートされない文字セット名が追加された場合は無視します。 その判定は String のコンストラクタ {@link String(byte[],String)}
   * を使用します。{@link Charset#isSupported} は JISAutoDetect の判定が 出来ないことや、JDK1.4.1 では ISO-2022-JP の場合
   * false を返すため 使用されません。
   * <p>
   * 
   * なお、要素を削除は今のところ考慮されておらず、削除した場合の動作は保証しません。
   */
  private class CharsetSortedModel extends DefaultComboBoxModel<Object> {
  ...
  /**
   * 動作プラットフォームからのテキストフィールドへのファイルのドロップを受け付ける ための転送ハンドラです。ファイルのドロップ以外の転送、例えば文字列のドロップや
    ...
    public boolean importData(JComponent comp, Transferable t) {
      ...
      if (t.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
        try {
          ...
          List<Object> files = (List<Object>) t.getTransferData(DataFlavor.javaFileListFlavor);
